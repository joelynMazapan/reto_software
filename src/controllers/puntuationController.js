...
const createPuntuation = async (req, res) => {
    try {
        ...
        if (!fkUsuario || !fkVideoJuego || puntos === undefined) {
            return res.status(400).json({ 
                msg: 'Los campos fkUsuario, fkVideoJuego y puntos son obligatorios.' 
            });
        }
        ...
        const [result] = await db.query(
            'INSERT INTO Puntuacion (fkUsuario, fkVideoJuego, Puntuacion) VALUES (?, ?, ?)',
            [fkUsuario, fkVideoJuego, puntos]
        );
